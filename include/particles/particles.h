...
template<uint32_t POS_DIMS>
__global__ void print_3d_particle_positions(uint32_t n_points, Eigen::Vector3i res_3d, Vectorxf<POS_DIMS>* __restrict__ out, uint32_t round) {
	uint32_t x = threadIdx.x + blockIdx.x * blockDim.x;
	uint32_t y = threadIdx.y + blockIdx.y * blockDim.y;
	uint32_t z = threadIdx.z + blockIdx.z * blockDim.z;
	if (x>=res_3d.x() || y>=res_3d.y() || z>=res_3d.z())
	...
	uint32_t i = x + y*res_3d.x() + z*res_3d.x()*res_3d.y();
    if(i >= n_points) return;
	printf("round,%d,i,%d,pos,%f,%f,%f\n", round, i, out[i].x(), out[i].y(), out[i].z());
}
